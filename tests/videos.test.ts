...
describe('videos resource - V2', () => {
  ...
  it('getById returns a single video', async () => {
    const res = await sdk.videos.getById(videoV2Resp.video.id);
    expect(res.id).toBe(videoV2Resp.video.id);
    expect(res.videoId).toBe(videoV2Resp.video.videoId);
  });
