...
describe("SDK initialization equivalence", () => {
  it("JaaqClient.init and createJaaqClient produce equivalent clients", () => {
    ...
    // Same public shape
    expect(Object.keys(clientFromInit)).toEqual(
      expect.arrayContaining(["videos", "collections"])
    );
    expect(Object.keys(clientFromFactory)).toEqual(
      expect.arrayContaining(["videos", "collections"])
    );
    ...
    expect(clientFromInit.constructor.name).toBe(
      clientFromFactory.constructor.name
    );
  });
});
